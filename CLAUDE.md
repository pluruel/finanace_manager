...
### How to Run Tests
- **Backend**: `DATABASE_URL=postgres://app:app@localhost:5432/postgres cargo test -p finance-manager` (admin URL — TestDb creates per-test ephemeral DBs and runs `Migrator::up`).
...
- 2026-05-16: import 페이지 폴더/다중 파일 업로드 — `(app)/import/page.tsx` 전면 재작성. `<input webkitdirectory>` + 드래그앤드롭(FileSystem `webkitGetAsEntry` 재귀 walk) + 다중 파일 picker 동시 지원, 클라이언트에서 `^\d{4}년\s*\d{1,2}월\.xlsx$` (NFC 정규화 후) 만 통과시켜 연/월 오름차순 정렬·중복 제거 후 순차로 기존 `POST /api/import` 호출. 백엔드/엔드포인트 변경 없음(SHA-256 dedup 으로 자연스럽게 멱등). 행마다 대기/업로드/완료/중복/실패 상태와 거래 삽입수·무결성·미해결 별칭 카운트를 인라인 표시. 백엔드 무변경, 프런트 테스트 151/151(import 페이지는 기존에도 테스트 없음 — 추가 없음).
- 2026-05-11: 일괄 클러스터 병합 — pg_trgm + GIN trgm 인덱스(`products_name_trgm_idx`/`merchants_name_trgm_idx`, `WITH SCHEMA pg_catalog` 로 sea-orm-migration 1.0.x 의 public-schema sweep 회피), `server/src/api/clusters.rs` 신규(`GET /api/clusters`, `POST /api/clusters/merge`). union-find 컴포넌트화 → 사용자가 `/aliases` 5번째 "클러스터" 탭에서 카드 단위로 대표/흡수 선택 → 단일 트랜잭션 SELECT FOR UPDATE + UPDATE transactions + DELETE aliases + DELETE absorbed entities. alias 학습 보존 X. 프론트는 `web/components/{cluster-card,cluster-tab}.tsx` + `web/lib/cluster-data.ts` + `web/app/api/clusters-proxy/{route.ts,merge/route.ts}`. 백엔드 96/96 (+9), 프런트 136/136 (+10). Spec/plan: `docs/superpowers/{specs,plans}/2026-05-11-bulk-cluster-merge*`.
