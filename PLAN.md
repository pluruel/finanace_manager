...
**Step D (✅ 2026-05-02)**: Frontend dashboard (summary + settlement card) at `web/app/(app)/page.tsx`.
- Month picker (`web/components/month-picker.tsx`, client) at top: prev/next buttons + native `<input type="month">`. State stored in `?ym=YYYY-MM` search param; default = current calendar month. Each section is wrapped in `<Suspense key={`section-${year}-${month}`}>` so changing the month re-fires the server fetch but does not re-mount sibling sections.
- **Settlement card** (`web/components/settlement-card.tsx`, server): renders "경비인정 ₩X − 차감 ₩X = 입금액 ₩X" from `GET /api/settlement/:year/:month`. When `recognized_expense = 0` AND `deducted_amount = 0` (or fetch returns null) it collapses to a `data-testid="settlement-empty"` message.
- **Category × Actor pivot** (`web/components/summary-pivot.tsx`, server): rows = categories, columns = actors in the order returned by the API ((미지정) included when present), KRW cells, right-edge `합계` column, bottom `합계` row + grand total. 차감 row is rendered with the muted-row treatment + "정산 차감" badge but otherwise totaled identically — the settlement card already separates it.
- **Recent transactions** (`(app)/page.tsx` inline, server): top 10 rows from `GET /api/transactions?from=<month-start>&to=<month-end>` reusing the existing list visuals (multi-line group badge, 차감 dimming). Group expand intentionally not used here since the dashboard treats each row as a summary entry; use `/transactions` for the full table.
- Loading: per-section `<Suspense>` with `CardSkeleton` (animated pulse) so individual sections can stream in independently.
- New schema: `SettlementSchema` extended with `year` / `month`; new `SummaryResponseSchema`, `CategorySummarySchema`, `ByActorEntrySchema`, `ActorRefSchema` mirror `server/src/api/summary.rs`.
