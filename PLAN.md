...
- **⚠ KNOWN ISSUE (pending backend fix)**: `concurrent_merges_one_winner` in `server/tests/test_m2_step_b.rs` is flaky in parallel mode (~40–50% fail rate). Root cause: the initial alias lookup in `handle_post_alias` at `server/src/api/aliases.rs:~352` does **not** use `SELECT ... FOR UPDATE`. Two simultaneous merges of the same alias can both read the alias before either commits, both compute `old_target_id`, and both proceed — the second performs an unintended secondary merge. The existing `SELECT FOR UPDATE` on the source entity row is insufficient because by the time it runs, both transactions have already determined their `old_target_id` from a stale read. **Fix**: add `FOR UPDATE` to the initial alias read in `handle_post_alias`. Re-run `cargo test -p server` 3× in parallel mode to verify. (User interrupted the dispatch of this fix; resume here on next session.)
